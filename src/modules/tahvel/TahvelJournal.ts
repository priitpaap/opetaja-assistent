      ...
      .addEventListener("click", async () => {
        ...
        if (dontAskAgain == null || confirmExistingGradesUpdate == null) {
          await TahvelJournal.showDialog({
            title: "Kas uuendan ka olemasolevaid hindeid?",
            content:
              "Kui õpilasel on juba ÕV hinded, siis kas uuendan neid ka automaatselt?",
            actions: [
              {
...
`
  ...
  static hideDialog() {
    document.getElementById("assistent-dialog").style.display = "none"
  }
  ...
  static async checkAndHighlightLearningOutcomesGrades() {
    ...
    const allLessonsAreInJournal =
      journal.contactLessonsInJournal - journal.contactLessonsPlanned >= -1

    // Get the last date from entriesInTimetable and format it
    const lastTimetableEntry =
      journal.entriesInTimetable[journal.entriesInTimetable.length - 1]
    const formattedDate = new Date(lastTimetableEntry.date).toLocaleDateString(
      "et-EE",
      {
        day: "2-digit",
        month: "2-digit",
    ...
    journal.learningOutcomes.forEach((outcome) => {
      ...
      const header = Array.from(document.querySelectorAll("th")).find((th) => {
        ...
        return (
          headerSpan &&
          headerSpan.textContent.trim().match(new RegExp(`^ÕV${outcomeCode}$`))
        )
      })
      ...
      journal.students.forEach((student) => {
        ...
        if (!studentRow) {
          console.log(
            `No matching row found for studentId ${student.studentId}`
          )
        ...
        if (!cell) {
          console.log(
            `No matching cell found for studentId ${student.studentId} at header index ${headerIndex}`
          )
        ...
        if (!allAssigmentsGraded) {
          if (allLessonsAreInJournal && !result) {
            ...
            cell.setAttribute(
              "data-tooltip",
              `Õpiväljundi hinded tuleb välja panna ${formattedDate}`
            )
            cell.classList.add("tooltip-container")
          } else {
            ...
            cell.setAttribute(
              "data-tooltip",
              `Antud ÕV-ga seotud tööd on vaja enne ära hinnata`
            )
            cell.classList.add("tooltip-container")
          }
  ...
  static async fetchJournalOutcome(
    journalId: number,
    outcomeId: number
    //eslint-disable-next-line
  ): Promise<any> {
  ...
  static insertAriaLabelText(journal) {
    ...
    journal.exercisesLists.forEach((exercise) => {
      ...
      if (learningOutcomes && learningOutcomes.length > 0) {
        ...
        const formattedLearningOutcomes = `${learningOutcomes
          .map((outcome) => `ÕV${outcome}`)
          .join(", ")}`
        ...
        if (matchingHeader) {
          const headerSpan = matchingHeader.querySelector(
            "span.journal-entry-button.pointer"
          ) as HTMLElement
          ...
          if (headerSpan) {
            ...
            if (innerSpan) {
              ...
              dateAndLearningOutcomesSpan.textContent = `${
                homeworkDuedate || "määramata"
              } ${formattedLearningOutcomes}`
              dateAndLearningOutcomesSpan.classList.add(
  ...
  static async addEventListenersToAddNewEntryButton() {
  ...
  static async insertKriitLinkinHomework(
    journalId,
    assignmentId,
    journalEntryId
  ) {
    ...
    const newEntry = {
      ...existingEntry,
      homework: `Kodutöö saab esitada siin: ${AssistentApiClient.kriitUrl}/assignments/${assignmentId}`
    }

    return await AssistentApiClient.put(
      `/journals/${journalId}/journalEntry/${journalEntryId}`,
      newEntry
    )
  }
}
