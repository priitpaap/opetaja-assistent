...
class AssistentDom {
      ...
      setTimeout(() => {
        if (!resolved) {
          ...
          if (targetElement) {
            ...
            reject(
              new Error(
                `Element ${selector} not found within ${
                  timeout / 1000
                } sec time limit`
              )
            )
  ...
  static async waitForElementToBeVisible(
      ...
      observer = new IntersectionObserver(
        (entries, observer) => {
          if (
            ...
            getComputedStyle(entries[0].target).display !== "none"
          ) {
            observer.disconnect()
            resolve(element)
    ...
    const timeoutPromise = new Promise<Element | null>((_, reject) => {
      setTimeout(() => {
        ...
        observer.disconnect()
        reject(
          new Error(`Timeout: Element ${s} not visible within ${timeout}ms`)
        )
      }, timeout)
    })
  ...
  static async waitForAttributeToAppear(
    element: Element,
    attribute: string,
    timeout: number = 3000
  ): Promise<string> {
    return new Promise<string>((resolve, reject) => {
      ...
      setTimeout(() => {
        ...
        reject(
          new Error(
            `${attribute} attribute not found within ${timeout} milliseconds`
          )
        )
      }, timeout)
    })
  ...
  static createStructure(html: string): HTMLElement | null {
    ...
    if (tempDiv.childNodes.length !== 1) {
      throw new AssistentDetailedError(
        500,
        "Invalid HTML",
        "HTML string must contain exactly one root element"
      )
    }
  ...
  static showErrorMessage(title: string, message: string, code: number): void {
    ...
    messageBox
      .querySelector(".assistent-message-box-close-button")
      .addEventListener("click", () => {
        messageBox.remove()
      })
  }

  static addDialogToDom() {
    document.body.appendChild(
      AssistentDom.createStructure(`<div id="assistent-dialog"></div>`)
    )
